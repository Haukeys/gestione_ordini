...
public class OrdineController {
    ...
    @PutMapping("/{id}/paga")
    public ResponseEntity<OrdineResponseDTO> payOrder(@PathVariable("id") UUID idOrdine) {

        OrdineResponseDTO response = ordineService.updateOrdine(idOrdine);

        // 2. Si le paiement est validé, on renvoie un statut 200 OK
        if (response.getStatoOrdine() == TipoOrdine.PAGATO) {
            return ResponseEntity.ok(response);
        }

        // 3. Si le paiement a échoué (DAPAGARE), le contrôleur lève manuellement une exception 402
